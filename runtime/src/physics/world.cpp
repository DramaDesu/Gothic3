...
bool hitTriangle(const std::array<float, 3> &origin, const std::array<float, 3> &direction,
                 ...
                 float &distance)
{
    ...
    const std::array<float, 3> t{origin[0] - a[0], origin[1] - a[1], origin[2] - a[2]};
    const float u = (t[0] * p[0] + t[1] * p[1] + t[2] * p[2]) * scale;
    if (u < 0.0f || u > 1.0f)
        return false;

    const std::array<float, 3> q{t[1] * ab[2] - t[2] * ab[1], t[2] * ab[0] - t[0] * ab[2],
                                 t[0] * ab[1] - t[1] * ab[0]};
...
void CollisionWorld::add(const genome::Mesh &mesh, const Matrix &world)
{
    ...
    const auto known = m_localBounds.find(&mesh);
    std::array<float, 6> local{};
    if (known != m_localBounds.end())
        local = known->second;
    else
    {
        local = {std::numeric_limits<float>::max(),  std::numeric_limits<float>::max(),
        ...
        m_localBounds.emplace(&mesh, local);
    }
    if (local[0] > local[3])
        return; // no vertices

    for (const genome::MeshElement &element : mesh.elements)
        m_triangles += element.indices.size() / 3;

    // The local box placed by its eight corners: cheaper than walking every
    // vertex again, and the result is that box or a little larger, which only
                                 ...
                                 std::array<float, 3> *normal) const
{
    ...
    {
        const Instance &instance = m_instances[index];
        // The column under the point, and only as far down as the ray goes.
        ...
        for (const genome::MeshElement &element : instance.mesh->elements)
        {
            for (std::size_t triangle = 0; triangle + 2 < element.indices.size(); triangle += 3)
            {
                const std::uint32_t a = element.indices[triangle];
                const std::uint32_t b = element.indices[triangle + 1];
                const std::uint32_t c = element.indices[triangle + 2];
                if (a >= element.positions.size() || b >= element.positions.size() ||
                    c >= element.positions.size())
                    continue;

                float distance = 0.0f;
                if (!hitTriangle(origin, direction, element.positions[a], element.positions[b],
                                 element.positions[c], distance) ||
                    distance >= best)
                    continue;
                ...
                if (normal != nullptr)
                {
                    hitCorners[0] = place(instance.world, element.positions[a]);
                    hitCorners[1] = place(instance.world, element.positions[b]);
                    hitCorners[2] = place(instance.world, element.positions[c]);
                }
            }
        }
    }
